...
def run_sync(start_ar: datetime, end_ar: datetime) -> dict:
    ...
    try:
        ...
        for e in eventos_raw:
            ...
            nombre = e["name"] or users.get(e["employee_no"], "")
            acc.append({
                "device":      e["device"],
        ...
        for (emp, fecha), evs in grupos.items():
            ...
            ci = evs[0]["_dt_ar"]
            co = evs[-1]["_dt_ar"] if len(evs) > 1 else None
            minutos = int((co - ci).total_seconds() // 60) if co else None
            devices = ",".join(sorted({e["device"] for e in evs}))
            filas.append({
                "employee_no":   emp,
                "employee_name": evs[0]["name"],
                "fecha":         fecha,
                "check_in":      ci.astimezone(timezone.utc),
